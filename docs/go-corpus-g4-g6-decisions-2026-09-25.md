...
| key | exact cause |
|---|---|
| `fixedbugs/bug260.go` (G6) | `fmt.Sprintf("%p", &b1[0])` vs `&b1[1]`: element pointers of an interpreter array cross to the native `fmt` as independent descriptors, so their addresses are not `base + i*sizeof(T)`. Needs a layout-consistent address model for interpreter arrays; no unsafe, no GC observation — not one of the seven families. Exact mechanism in round 2 |
| `nilptr.go` (G6) | memory only, after sh `993ac3e2` (round 2 below): `var dummy [256 << 20]byte` is a dense `[]any` payload plus a `meta.sequence` slot per element (24 B/element, ~6 GiB reserved, 4.3 GB peak on darwin; `fatal error: runtime: out of memory` on the leaf). All sixteen probes now panic as gc does and the root is rc=0 locally. The fix is a compact backing store for scalar arrays, which touches every `[]any` consumer (~140 sites in `interp`) — see round 2 |
...
| `nilptr.go` (G6) | open (memory) | p3/p6/p13 printed the internal `scalar call interrupted` sentinel to stderr when a typed `var` / `:=` slice initializer panicked; p14–p16 failed `BASHPP-ECOLLECTION-SLICE` because `(*[1]int)(nil)` reaches the slice path as a value-less pointer carrying only pointer metadata. Both fixed (`993ac3e2`, test `TestS270G6NilArrayPointerSlicePanics`); the whole root is rc=0 locally in 1.2 s. What remains is the 256 Mi-element array. Arrays are `[]any` + `[]*bashPPCollectionMeta` everywhere (`bashPPCollectionZero`); a lazily materialised or typed backing store has no single choke point (`vr.Obj` / `.([]any)` are read directly at ~140 sites), so it was not attempted inside this lane's budget next to the concurrent G1 interp lane. Not excludable: the root asserts nil-check semantics, not a runtime observation |
| `fixedbugs/bug260.go` (G6) | open (address model) | a pointer argument crosses to the dependency worker with a transport origin keyed by (root cell, element path) (`bashPPTransportOrigin`); the worker allocates one native pointee per origin, so `&b1[0]` and `&b1[1]` are two unrelated native allocations (stable across calls, but not `base + i*size`). A layout-consistent `%p` needs the worker to materialise the root array once per root origin and hand out `&root[i]`, with element writeback going through the root — a bridge protocol change (pointer value carrying parent origin + index), not attempted here. No `unsafe` in the source, so `unsafe-reinterpretation` does not apply |
